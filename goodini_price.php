...
$sql_main = 'SELECT *
	...
	WHERE p.status = \'1\' AND pd.language_id=\'5\' AND p.quantity > 0
	GROUP BY pd.name ORDER BY LCASE(pd.name) ASC';
...
$sql="SELECT *
	...
	WHERE p.status = '1' AND p.date_available <= NOW() AND p.quantity > 0 AND p2c.main_category = 1 AND pd.language_id=5 AND pd.name = '{$main_row['name']}'";
$query = $db->query($sql);
			...
			if($query5->rows) {
				foreach ($query5->rows as $row5) {
					...
					foreach ($query6->rows as $row6) {
						...
						$price[] = '<offer id="'.$row['sku'].'_'.$row6['name'].'" available="true" group_id="'.$group_id.'">';
...
echo join('',$price);
echo "<pre>";
print_r($main_query->rows);
echo "</pre>";
*/

$file = 'lotto_price_goodini.xml';
