...
<firmName>Официальный интернет магазин LOTTO</firmName>
<firmId>27424</firmId>
<categories>';
...
foreach ($query->rows as $result) {
	if($result['category_id'] != 59 && $result['category_id'] != 233 && $result['category_id'] != 226 && $result['category_id'] != 227) {
		$price[] = '<category>
		<id>'.$result['category_id'].'</id>';
			if($result['parent_id'] != 0){
				$price[] = '<parentId>'.$result['parent_id'].'</parentId>';
			}
		$price[] = '<name>'.$result['name'].'</name>
		</category>';
	}
}
$price[] = '</categories>
...
$sql_main = 'SELECT name FROM '. DB_PREFIX .'product_description GROUP BY name ORDER BY name ASC, LCASE(name) ASC';
$main_query = $db->query($sql_main);

$group_id = 1;

foreach ($main_query->rows as $main_row) {
	
$main_row['name'] = addslashes($main_row['name']);
...
$sql="SELECT *
	...
	WHERE p.status = '1' AND p.date_available <= NOW() AND p2c.main_category = 1 AND pd.language_id=5 AND p.quantity > 0 AND pd.name = '{$main_row['name']}'";
$query = $db->query($sql);
foreach ($query->rows as $row) {
	if(count($query->rows) > 1){
		$group_id_switch = TRUE;
	} else {
		$group_id_switch = FALSE;
	}
		...
		foreach ($query2->rows as $row2) {
			...
			if($query5->rows) {
					...
					foreach ($query6->rows as $row6) {
						switch($option_id){
							...
							case 15: switch($option_value_id){	 	//размер носков
										case 127: $row6['name'] = "XS"; break;
										case 128: $row6['name'] = "S"; break;
										case 129: $row6['name'] = "M"; break;
										case 130: $row6['name'] = "L"; break;
										case 131: $row6['name'] = "XL"; break;
										}
									 $size_unit = "INT"; break;
						...
						$price[] = '<item>
						<id>'.$row['sku'].$row6['name'].'</id>
						<categoryId>'.$row['category_id'].'</categoryId>
						<group_id>'.$group_id.'</group_id>
						<vendor>LOTTO</vendor>
						<code>'.$row['sku'].'</code>
						<name>'.$row['name'].'</name>
						<description>'.strip_tags(preg_replace("/Перейдя на страницу.*(.*)этой категории\.?/i", "", $row['description'])).'</description>
						<url>'.HTTP_SERVER.''.$row2['keyword'].'/'.$row1['keyword'].'</url>
						<image>'.HTTP_SERVER.'image/'.$row['image'].'</image>';
						...
						foreach ($query3->rows as $row3) {
							$price[] = '<image>'.HTTP_SERVER.'image/'.$row3['image'].'</image>';
						}
						...
						if($query4->rows) {
							foreach ($query4->rows as $row4) {
								$price[]='<priceRUAH>'.(int)$row4['price'].'</priceRUAH>';
							}
							$price[] = '<oldprice>'.(int)$row['price'].'</oldprice>';
						} else {
							$price[]='<priceRUAH>'.(int)$row['price'].'</priceRUAH>';
						}
						
						$price[] = '<stock>В наличии</stock>';
						
						$price[] = '<param name="Размер" unit="'.$size_unit.'">'.$row6['name'].'</param>';
						...
						$query7 = $db->query($sql7);
							if($query7->rows) {
								foreach ($query7->rows as $row7) {
									if(strripos($row7['text'], "/") != FALSE) {
										$row7['text'] = explode("/", $row7['text']);
										$price[]='<param name="Цвет">'.$row7['text'][0].'</param>';
									} else {
						...
						//  GETTING PRODUCT ATTRIBUTES: TYPE				
						$sql9 = 'SELECT text FROM '. DB_PREFIX .'product_attribute WHERE product_id = '.$row['product_id'].' AND attribute_id = 13 AND language_id = 5';
						$query9 = $db->query($sql9);
							if($query9->rows) {
								foreach ($query9->rows as $row9) {
									$price[]='<param name="Тип">'.$row9['text'].'</param>';
								}
							}
				...
				$price[] = '<item>
						...
						<categoryId>'.$row['category_id'].'</categoryId>';
						if ($group_id_switch) {
							$price[] = '<group_id>'.$group_id.'</group_id>';
						} 
							$price[] = '<vendor>LOTTO</vendor>
										<code>'.$row['sku'].'</code>
										<name>'.$row['name'].'</name>
										<description>'.strip_tags(preg_replace("/Перейдя на страницу.*(.*)этой категории\.?/i", "", $row['description'])).'</description>
										<url>'.HTTP_SERVER.''.$row2['keyword'].'/'.$row1['keyword'].'</url>
