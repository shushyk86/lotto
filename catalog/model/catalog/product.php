...
class ModelCatalogProduct extends Model {
	...
	public function updateViewed($product_id) {
		...
		$this->db->query("UPDATE " . DB_PREFIX . "product_views SET viewed = (viewed + 1) WHERE product_id = '" . (int)$product_id . "'");
	}
	...
	// Start coolfilter
		...
		$sql .= " LEFT JOIN " . DB_PREFIX . "product_views pv ON (p.product_id = pv.product_id)";

		$sql .= " LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id) LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'";

		if(isset($data['fsc']) && $data['fsc']) {
			$sql .= " AND p2c.product_id NOT IN (SELECT p2c2.product_id FROM oc_product_to_category p2c2 WHERE p2c2.category_id = '" . (int)$data['fsc'] . "')";
		}
		...
		if (isset($data['order']) && ($data['order'] == 'DESC')) {
			if (isset($data['sort']) && in_array($data['sort'], $sort_data)) {
				if ($data['sort'] == 'pv.viewed') {
					$sql .= " DESC, (CASE WHEN special IS NOT NULL THEN special WHEN discount IS NOT NULL THEN discount ELSE p.price END) DESC, LCASE(pd.name) ASC";
				} else {
...
*/
	...
	public function hasSaleCategory($product_id) {
