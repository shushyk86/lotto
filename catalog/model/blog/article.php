...
class ModelBlogArticle extends Model {
	...
	public function getArticle($article_id) {
		...
		if ($query->num_rows) {
            $date = explode('-', $query->row['date_added']);
			...
			return array(
				...
				'date_added'       => $date[2].'.'.$date[1].'.'.$date[0],
	...
	public function getArticles($data = array()) {
		...
		if (!$article_data) {
            ...
            if (!empty($data['filter_article_tag'])) {
                $sql .= " AND pd.tag LIKE '%" . $data['filter_article_tag'] . "%'";
            }
    ...
    public function getRecommendedArticles() {
        ...
        $query = $this->db->query("SELECT p.article_id FROM " . DB_PREFIX . "article p LEFT JOIN " . DB_PREFIX . "article_to_store p2s ON (p.article_id = p2s.article_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p.banner_top = 1 AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY p.viewed DESC, p.date_added DESC");
