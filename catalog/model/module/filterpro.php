...
class ModelModuleFilterPro extends Model {
    ...
    public function getAttributes($data) {
        ...
        $sql = "SELECT DISTINCT pa.text, p.quantity, a.`attribute_id`, ad.`name`, ag.attribute_group_id, agd.name as attribute_group_name FROM `" . DB_PREFIX . "product_attribute` pa" .
    ...
    public function getProducts($data) {
        ...
        $sql .= "SELECT DISTINCT p.product_id, pd.name, p.model, p.sku, p.quantity, p.price, p.sort_order, p.date_added, pv.viewed";
        ...
        $sql .= "FROM " . DB_PREFIX . "product p" .
            ...
            " LEFT JOIN " . DB_PREFIX . "product_views pv ON (pv.product_id=p.product_id)" .
            " LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p2s.product_id=p.product_id)" .
            " LEFT JOIN " . DB_PREFIX . "product_to_category p2c ON (p2c.product_id=p.product_id)";
        if($data['attribute_value'] || $data['attr_slider']) {
            $sql .= " LEFT JOIN " . DB_PREFIX . "product_attribute p2a ON (p2a.product_id=p.product_id)";
        ...
        if(isset($data['order']) && ($data['order'] == 'DESC')) {
            if ($data['sort'] == 'viewed') {
                $sql .= " DESC, realprice DESC, LCASE(name) ASC, LCASE(sku) ASC";
            } else {
