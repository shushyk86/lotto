...
class ModelCheckoutOrder extends Model {
	...
	public function getOrder($order_id) {
		...
		$custom_order_info = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_product` WHERE order_id = '" . (int)$order_id . "'");
		...
		if ($order_query->num_rows) {
			...
			return array(
				...
				'product_name'            => $custom_order_info->row['name'],
				'product_quantity'        => $custom_order_info->row['quantity'],
				'product_price'           => $custom_order_info->row['price'],
				'product_id'              => $custom_order_info->row['product_id']
			);
		} else {
			return false;	
		}
	...
	public function confirm($order_id, $order_status_id, $comment = '', $notify = false) {
		...
		if ($order_info && !$order_info['order_status_id']) {
			...
			// Comment
			$order_comment_query = $this->db->query("SELECT comment FROM " . DB_PREFIX . "order WHERE order_id = '" . (int)$order_id . "'");

			if (!empty($order_comment_query->row['comment'])){
				$customer_comment = $order_comment_query->row['comment'];
			} else {
			...
			$template->data['customer_comment'] = $customer_comment;
			$template->data['date_added'] = date('d.m.Y', strtotime($order_info['date_added']));
			$template->data['payment_method'] = $order_info['payment_method'];
			$template->data['shipping_method'] = $order_info['shipping_method'];
			$template->data['email'] = $order_info['email'];
			$template->data['telephone'] = $order_info['telephone'];
			$template->data['ip'] = $order_info['ip'];
			$template->data['admin_alert'] = false;
			...
			if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/mail/order_customer.tpl')) {
				$html = $template->fetch($this->config->get('config_template') . '/template/mail/order_customer.tpl');
			} else {
				$html = $template->fetch('default/template/mail/order_customer.tpl');
			}
			...
			if ($this->config->get('config_alert_mail')) {
				...
				$template->data['admin_alert'] = true;
...
// Send data to GetResponse
			if(isset($order_info['email']) && $order_info['email'] != $this->config->get('config_email') && $order_info['email'] != 'zx@bigmir.net' && $order_info['email'] != 'lottoukraine@gmail.com' && $order_info['email'] != 'lotto@gmail.com' && $order_info['email'] != 'zx@gmail.com') {
				$this->load->library('getresponse');

				$gr_name = $order_info['shipping_firstname'] . (!empty($order_info['shipping_lastname']) ? ' '.$order_info['shipping_lastname'] : '');
				$gr_email = $order_info['email'];

				$client = new Getresponse();
				$client->sendContact($gr_name, $gr_email, $this->gr_campaign);
			}
