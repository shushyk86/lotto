...
class ModelCheckoutCoupon extends Model {
	public function getCoupon($code) {
		...
		if ($coupon_query->num_rows) {
			...
			if(isset($coupon_query->row['all_product']) && $coupon_query->row['all_product'] && $coupon_product_data){
				foreach ($this->cart->getProducts() as $product) {
					if (in_array($product['product_id'], $coupon_product_data)) {
						$all_product_data[] = $product['product_id'];
						continue;
					}
				}
				if (!$all_product_data || sizeof($all_product_data)!=sizeof($coupon_product_data)) {
					$status = false;
				}
			}
