...
<div id="check_data">
  <div class="middle" id="smch_modal_data">
    ...
    <!-- NAME and ANOTHER DATA -->
      ...
      <div class="name">
        ...
        <?php if ( $product_model && isset( $hide_product_model ) ) { ?><span> <?php echo $product_sku; ?></span><?php } ?>
      ...
      <div class="quantity" id="smch_quantity">
        <div>
          <button onclick="$(this).next().val(~~$(this).next().val()-1); update_quantity('<?php echo $product_id; ?>');" id="decrease_quantity">&mdash;</button>
          <input 
            type="text" 
            name="quantity" 
            value="1" 
            ...
            id="oc_input_quantity"
          />
          <button onclick="$(this).prev().val(~~$(this).prev().val()+1); update_quantity('<?php echo $product_id; ?>');" id="increase_quantity">+</button>
  ...
  <div class="bottom">
    <?php if ( $price ) { ?>
      ...
      <?php } ?>
    <?php } ?>
    <input type="button" id="but-to" onclick="sendOrder();" value="<?php echo $button_send_order; ?>" />
  </div>
</div>
...
<?php } ?>
<?php $actual_price = ($special ? $special : $price);
...
$('#but-to').on('click', function() {
  setTimeout(function() {
    if($('#but-to').css('display') != 'block') {
      $('#smch_modal_body').css('border', 'none');
      $('#smch_modal_data').addClass('updated');
    }
  }, 1500)
});

$('.top .close_button').on('click', function() {
  $('#smch_modal_body').hide();
});
