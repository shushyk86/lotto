        ...
        if (isset($this->request->get['blid'])) {
            ...
            if ($news_info) {
                ...
                $this->data['sorts'][] = array(
                    ...
                    'value' => 'pd.name-ASC',
                    'href'  => $this->url->link('blog/news', '&sort=pd.name&order=ASC' . $url)
                );

                $this->data['sorts'][] = array(
                    'text'  => $this->language->get('text_sort_date'),
                    'value' => 'p.date_added-DESC',
                    'href'  => $this->url->link('blog/news', '&sort=p.date_added&order=DESC' . $url)
                );

                //ocshop sort viewed

                $this->data['sorts'][] = array(
                    'text'  => $this->language->get('text_sort_viewed'),
                    'value' => 'p.viewed-DESC',
                    'href'  => $this->url->link('blog/news', '&sort=p.viewed&order=DESC' . $url)
                );
                //ocshop sort viewed
                ...
                $this->data['continue'] = $this->url->link('blog/news');
            ...
            $this->data['news_id'] = $news_id = 0;
            $news_info = $this->model_blog_news->getCategories($news_id);
            ...
            if ($news_info) {
                ...
                $results = $this->model_blog_article->getRecommendedArticles();

                foreach ($results as $result) {
                    if ($result['image']) {
                        $image = $this->model_tool_image->resize($result['image'], 535, 400);
                    } else {
                        $image = false;
                    }

                    if ($result['image']) {
                        $image_gallery = $this->model_tool_image->resize($result['image'], 535, 400);
                    } else {
                        $image_gallery = false;
                    }
                    ...
                    $this->data['latest_articles'][] = array(
                        ...
                        'name'              => $result['name'],
                        'description'       => utf8_substr(strip_tags(html_entity_decode($result['description'], ENT_QUOTES, 'UTF-8')), 0, 500) . '...',
                        'description_mini'  => strip_tags(html_entity_decode($result['description_mini'], ENT_QUOTES, 'UTF-8')),
                        'date_added'        => date($this->language->get('date_format_short'), strtotime($result['date_added'])),
                        'href'              => $this->url->link('blog/article',  '&article_id=' . $result['article_id'])
                    );
                }
                ...
                $this->data['limits'][] = array(
                    'text'  => $this->config->get('config_blog_catalog_limit'),
                    'value' => $this->config->get('config_blog_catalog_limit'),
                    'href'  => $this->url->link('blog/news', $url . '&limit=' . $this->config->get('config_blog_catalog_limit'))
                );

                $this->data['limits'][] = array(
                    'text'  => 25,
                    'value' => 25,
                    'href'  => $this->url->link('blog/news', $url . '&limit=25')
                );

                $this->data['limits'][] = array(
                    'text'  => 50,
                    'value' => 50,
                    'href'  => $this->url->link('blog/news', $url . '&limit=50')
                );

                $this->data['limits'][] = array(
                    'text'  => 75,
                    'value' => 75,
                    'href'  => $this->url->link('blog/news', $url . '&limit=75')
                );

                $this->data['limits'][] = array(
                    'text'  => 100,
                    'value' => 100,
                    'href'  => $this->url->link('blog/news', $url . '&limit=100')
                );
                ...
                $pagination->limit = $limit;
                $pagination->text = $this->language->get('text_pagination');
                $pagination->url = $this->url->link('blog/news', $url . '&page={page}');
