...
class ControllerCheckoutSimpleCheckoutPayment extends Controller {
    public function index() {
       ...
       foreach ($results as $result) {
        ...
        $this->data['total_sum'] = $total;
        $this->data['results'] = $total_data;
		$this->data['sub_total'] = $total_data[0]['value'];

// end -entrance
//$this->data['quote']= $taxes;
	...
	//
				$tdat = array();
				$this->cur_payment_method = $method;
				$this->load->model('total/payment_discounts1');
				$this->model_total_payment_discounts1->getTotal($tdat, $subt, $taxes);
				$this->data['quote'][$result['code']]['name'] = $method;
				$this->data['quote'][$result['code']]['sum'] = $tdat[0]['text'];
//
                    if ($method) {
        ...
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && isset($this->request->post['payment_method']) && !empty($this->data['payment_methods'][$this->request->post['payment_method']])) {
            ...
            /* 
                 if ($this->config->get($result['code'] . '_status')) {
                    ...
                    $this->data['quote'][] = $this->{'model_total_' . $result['code']}->getTotal($total_data, $total, $taxes);
                }
            */
