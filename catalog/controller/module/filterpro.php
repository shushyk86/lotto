	...
	protected function index($setting) {
		if($setting['type'] == 1) {
			...
			$this->data['sorts'][] = array(
				...
				'href' => $this->url->link('product/filter', '&sort=pv.viewed&order=DESC' . $url)
			);
   ...
   foreach($attribute_group['attribute_values'] as $attribute_id => $attribute) {
	...
	public function getProducts() {
        ...
        if ($category_info['seo_h1']) {
            $this->data['category_name'] = $category_info['seo_h1'];
        } else {
		...
		$result_html = $this->getHtmlProducts($results, $product_total, $category_id, $position);
	...
	private function getHtmlProducts($results, $product_total, $category_id, $position) {
