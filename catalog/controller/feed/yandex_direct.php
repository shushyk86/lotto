...
class ControllerFeedYandexDirect extends Controller {
	...
	public function index() {
        $this->load->model('export/yandex_direct');
        $this->load->model('localisation/currency');
        $this->load->model('tool/image');

        // Магазин
        $this->setShop('name', 'Официальный интернет магазин LOTTO');
        $this->setShop('company', 'LOTTO UKRAINE');
        ...
        // Категории
        $categories = $this->model_export_yandex_direct->getCategory();

        $allowed_categories = array();

        foreach ($categories as $category) {
            $this->setCategory($category['name'], $category['category_id'], $category['parent_id']);
            $allowed_categories[] = $category['category_id'];
        }

        // Товарные предложения
        $in_stock_id = 7; // id статуса товара "В наличии"
        $out_of_stock_id = 5; // id статуса товара "Нет на складе"
        $vendor_required = false; // true - только товары у которых задан производитель, необходимо для 'vendor.model'
        $products = $this->model_export_yandex_direct->getProduct(implode(',', $allowed_categories), $out_of_stock_id, $vendor_required);
...
//				$data['local_delivery_cost'] = 100;
            ...
            $data['description'] = strip_tags(preg_replace("/Перейдя на страницу.*(.*)этой категории\.?/i", "", $product['description']));
...
*/
	...
	private function setOffer($data) {
		...
		$attributes = array('id', 'type', 'available', 'param');
