...
/*Additional offer*/
	...
	public function resetViews(){
		...
		if($this->request->server['REQUEST_METHOD'] == 'POST' && $this->model_catalog_product->resetViews()){
			$json['success'] = 'Готово!';
		} else {
			$json['error'] = 'Произошла ошибка при сбросе счетчика просмотров!';
		}
	...
	protected function getList() {
		...
		$this->data['sort_model'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=p.sku' . $url, 'SSL');
		...
		$this->data['sort_views'] = $this->url->link('catalog/product', 'token=' . $this->session->data['token'] . '&sort=pv.viewed' . $url, 'SSL');
	...
	protected function getForm() {
		...
		$this->data['entry_sizetype'] = $this->language->get('entry_sizetype');		
		...
		$this->data['sizetypes'] = array(
				'0'=>'--Не выбрано--',
				'1'=>'Выбор р-ра обуви взрослой',
				'2'=>'Выбор р-ра обуви детской',
				'3'=>'Выбор р-ра одежды женский',
				'4'=>'Выбор р-ра одежды мужской',
				'5'=>'Выбор р-ра одежды детской',												
				'6'=>'Выбор р-ра носков',												
				'7'=>'Выбор р-ра гетр'												
				);
		...
		if (isset($this->request->post['sizetype'])) {
			$this->data['sizetype'] = $this->request->post['sizetype'];
		} elseif (!empty($product_info)) {
	...
	public function autocomplete() {
		...
		if (isset($this->request->get['filter_name']) || isset($this->request->get['filter_model']) || isset($this->request->get['filter_category_id'])) {
			...
			foreach ($results as $result) {
				...
				$json[] = array(
					'product_id' => $result['product_id'],
					'name'       => strip_tags(html_entity_decode($result['sku'], ENT_QUOTES, 'UTF-8')).' - '.strip_tags(html_entity_decode($result['name'], ENT_QUOTES, 'UTF-8')),	
					'model'      => $result['model'],
					'option'     => $option_data,
					'price'      => $result['price']
				);	
