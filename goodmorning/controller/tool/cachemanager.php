...
class ControllerToolCachemanager extends Controller {
	...
	public function index() {   
		...
		$this->data['image_description_suppler'] = $this->language->get('image_description_suppler');
		...
		$this->data['button_clearcache_suppler'] = $this->language->get('button_clearcache_suppler');
		...
		$this->data['clearcache_suppler'] = (HTTPS_SERVER . 'index.php?route=tool/cachemanager/clearcache_suppler&token=' . $this->session->data['token']);
	...
	public function clearcache() {
		...
		foreach($imgfiles as $imgfile){
			if(is_dir($imgfile)){
				$this->deldir($imgfile);
			} elseif(is_file($imgfile)){
				@unlink($imgfile);
			}
		}
		
		$error_log_file = glob(DIR_SYSTEM . 'logs/error.txt');
		if (is_file($error_log_file[0])) {
			@unlink($error_log_file[0]);
		}
		$this->session->data['success'] = $this->language->get('text_success');
	...
	public function clearcache_suppler() {
	
		$this->load->language('tool/cachemanager');
		
        $imgfiles = glob(DIR_IMAGE . 'data/photo/*');
            foreach($imgfiles as $imgfile){
				if(is_dir($imgfile)){
                    $this->deldir($imgfile);
				} elseif(is_file($imgfile)){
	...
	public function deldir($directory) {
        if (file_exists($directory)) {
            ...
            echo "Не удалось удалить кэш изображений";
        }
    }
