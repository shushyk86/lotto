...
class ControllerSaleOrder extends Controller {
	...
	protected function getList() {
		...
		if (isset($this->request->get['filter_bonus'])) {
			$filter_bonus = $this->request->get['filter_bonus'];
		} else {
			$filter_bonus = null;
		}
		...
		if (isset($this->request->get['filter_bonus'])) {
			$url .= '&filter_bonus=' . $this->request->get['filter_bonus'];
		}
        ...
        if(isset($this->request->get['filter_bonus'])){
            $data = array(
                ...
                'filter_customer_id'	 => true,
                'filter_order_status_id' => 5,
                'filter_total'           => $filter_total,
                'filter_date_added'      => $filter_date_added,
                'filter_date_modified'   => $filter_date_modified,
                'filter_date_max'        => date('Y-m-d H:i:s', strtotime('-' . (int)$this->config->get('config_order_edit') . ' day')),
                'sort'                   => $sort,
                'order'                  => $order
            );
        } else {
		...
		foreach ($results as $result) {
		    if(isset($this->request->get['filter_bonus'])) {
                ...
                $date_start = $this->model_sale_order->getOrderHistoryForBonus($result['order_id']);
                ...
                if(!empty($date_start)) {
                    ...
                    if($order_info['reward']){
                        if((($now - $date_start_unix) > 1209600) && !$reward_total){
                            $action = array();
...
/*
        if(isset($this->request->get['filter_bonus'])){
            $order_total = count($this->data['orders']);
        }
*/
		...
		$this->data['text_bonus_not_added'] = $this->language->get('text_bonus_not_added');
		...
		$this->data['filter_bonus'] = $filter_bonus;
	...
	public function getForm() {
		...
		if (isset($this->request->post['fax'])) {
			...
			switch($order_info['fax']) {
				case "value1": $order_info['fax'] = "«Макрос»"; break;
				case "value2": $order_info['fax'] = "с.Петровское"; break;
				case "value3": $order_info['fax'] = "Осокорки"; break;
                case "value4": $order_info['fax'] = "Глобус"; break;
			}
