...
//ocshop benefits
	...
	public function getProducts($data = array()) {
		$sql = "SELECT p.*, pd.*, pv.viewed, m.name as 'm_name' FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_description pd ON (p.product_id = pd.product_id)";
		...
		if (!empty($data['filter_name'])) {
			$sql .= " AND pd.name LIKE '%" . $this->db->escape($data['filter_name']) . "%'  OR p.sku LIKE '%" . $this->db->escape($data['filter_name']) . "%'";
		}
	...
	public function resetViews(){
		if ($this->db->query("UPDATE " . DB_PREFIX . "product_views SET viewed = '0'")){
			return true;
		}
	}
		//ocshop benefits
