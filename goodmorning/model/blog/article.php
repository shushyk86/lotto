...
class ModelBlogArticle extends Model {
	public function addArticle($data) {
		...
		$this->db->query("INSERT INTO " . DB_PREFIX . "article SET article_review = '" . (int)$data['article_review'] . "', status = '" . (int)$data['status'] . "', gstatus = '" . (int)$data['gstatus'] . "', banner_top = '" . (isset($data['banner_top']) ? (int)$data['banner_top'] : 0) . "', sort_order = '" . (int)$data['sort_order'] . "', date_added = NOW()");
		...
		foreach ($data['article_description'] as $language_id => $value) {
			$this->db->query("INSERT INTO " . DB_PREFIX . "article_description SET article_id = '" . (int)$article_id . "', language_id = '" . (int)$language_id . "', name = '" . $this->db->escape($value['name']) . "', meta_keyword = '" . $this->db->escape($value['meta_keyword']) . "', meta_description = '" . $this->db->escape($value['meta_description']) . "', description = '" . $this->db->escape($value['description']) . "', description_mini = '" . $this->db->escape($value['description_mini']) . "', seo_title = '" . $this->db->escape($value['seo_title']) . "', tag = '" . $this->db->escape($value['tags']) . "', seo_h1 = '" . $this->db->escape($value['seo_h1']) . "'");
		}
		...
		if (isset($data['article_image'])) {
			foreach ($data['article_image'] as $article_image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "article_image SET article_id = '" . (int)$article_id . "', image = '" . $this->db->escape(html_entity_decode($article_image['image'], ENT_QUOTES, 'UTF-8')) . "', link = '" . $this->db->escape($article_image['link']) . "', sort_order = '" . (int)$article_image['sort_order'] . "'");
			}
		}
        ...
        if (isset($data['product_category'])) {
            foreach ($data['product_category'] as $product_category_id) {
                $this->db->query("INSERT INTO " . DB_PREFIX . "article_related_wb SET article_id = '" . (int)$article_id . "', category_id = '" . (int)$product_category_id . "'");
            }
        }
	...
	public function editArticle($article_id, $data) {
        $this->db->query("UPDATE " . DB_PREFIX . "article SET article_review = '" . (int)$data['article_review'] . "', status = '" . (int)$data['status'] . "', sort_order = '" . (int)$data['sort_order'] . "', gstatus = '" . (int)$data['gstatus'] . "', banner_top = '" . (isset($data['banner_top']) ? (int)$data['banner_top'] : 0) . "', date_modified = NOW() WHERE article_id = '" . (int)$article_id . "'");
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "article_related_wb WHERE article_id = '" . (int)$article_id . "'");
    ...
    public function getArticleRelatedCategories($article_id) {
