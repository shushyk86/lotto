...
class ModelSaleOrder extends Model {
	...
	public function getOrders($data = array()) {
        ...
        if (!empty($data['filter_date_max'])) {
            $sql .= " AND DATE(date_modified) > DATE('" . $this->db->escape($data['filter_date_max']) . "')";
        }
    ...
    public function getOrderHistoryForBonus($order_id) {
        $query = $this->db->query("SELECT MAX(date_added) AS date_start FROM " . DB_PREFIX . "order_history WHERE order_id = '" . (int)$order_id . "' AND order_status_id = '5'");
