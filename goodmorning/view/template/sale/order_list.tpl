...
<!-- Бонусные гривны -->
              <td align="right"><div style="float: left"><input type="checkbox" <?php if ($filter_bonus) { ?>checked="checked"<?php } ?> name="bonus" id="bonus"><label for="bonus"><?php echo $text_bonus_not_added; ?></label></div><a onclick="filter();" class="button"><?php echo $button_filter; ?></a></td>
<!-- Бонусные гривны -->
            </tr>
            <?php if ($orders) { ?>
...
function filter() {
	...
	var filter_bonus = $('input[name=\'bonus\']').prop('checked');

    if (filter_bonus) {
        url += '&filter_bonus=' + encodeURIComponent(filter_bonus);
    }
